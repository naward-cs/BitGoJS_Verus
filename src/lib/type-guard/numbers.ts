...
export function isInt8(value: unknown) {
  return isInteger(value) && (value << 24) >> 24 === value
}

export function isInt16(value: unknown) {
  return isInteger(value) && (value << 16) >> 16 === value
}

export function isInt32(value: unknown) {
  return isInteger(value) && (value | 0) === value
}

const INT53_MAX = Math.pow(2, 53) - 1
export function isInt53(value: unknown) {
  return (
    isInteger(value) &&
    value >= 0 &&
    value <= INT53_MAX &&
...
export function isUInt8(value: unknown) {
  return isUInteger(value) && (value & 0xff) === value
}

export function isUInt16(value: unknown) {
  return isUInteger(value) && (value & 0xffff) === value
}

const UINT31_MAX = Math.pow(2, 31) - 1
export function isUInt31(value: unknown) {
  return isUInteger(value) && isUInt32(value) && value <= UINT31_MAX
}

export function isUInt32(value: unknown) {
  return isUInteger(value) && value >>> 0 === value
}

export function isUInt53(value: unknown) {
  return (
    isUInteger(value) &&
    value >= 0 &&
    value <= INT53_MAX &&
...
export function range(a: number, b: number, fn: ValidFn = isNumber) {
  function _range(value: unknown, strict?: boolean) {
    if (strict) {
