...
describe('bitcoinjs-lib (BIP32)', function () {
  ...
  it('can create a BIP32, bitcoin, account 0, external address', function () {
    var path = "m/0'/0/0"
    var root = bitcoin.HDNode.fromSeedHex(
  ...
  it('can use BIP39 to generate BIP32 addresses', function () {
    ...
    // receive addresses
    assert.strictEqual(
      root.derivePath("m/0'/0/0").getAddress(),
      '1AVQHbGuES57wD68AJi7Gcobc3RZrfYWTC',
    )
    assert.strictEqual(
      root.derivePath("m/0'/0/1").getAddress(),
      '1Ad6nsmqDzbQo5a822C9bkvAfrYv9mc1JL',
    )

    // change addresses
    assert.strictEqual(
      root.derivePath("m/0'/1/0").getAddress(),
      '1349KVc5NgedaK7DvuD4xDFxL86QN1Hvdn',
    )
    assert.strictEqual(
      root.derivePath("m/0'/1/1").getAddress(),
