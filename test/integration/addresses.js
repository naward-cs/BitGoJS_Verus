...
describe('bitcoinjs-lib (addresses)', function () {
  ...
  it('can generate a SegWit address (via P2SH)', function () {
    ...
    var witnessScript = bitcoin.script.witnessPubKeyHash.output.encode(
      bitcoin.crypto.hash160(pubKey),
    )
    var scriptPubKey = bitcoin.script.scriptHash.output.encode(
  ...
  it('can generate a SegWit 3-of-4 multisig address', function () {
    ...
    var scriptPubKey = bitcoin.script.witnessScriptHash.output.encode(
  ...
  it('can generate a SegWit 2-of-2 multisig address (via P2SH)', function () {
    ...
    var redeemScript = bitcoin.script.witnessScriptHash.output.encode(
      bitcoin.crypto.sha256(witnessScript),
    )
    var scriptPubKey = bitcoin.script.scriptHash.output.encode(
      bitcoin.crypto.hash160(redeemScript),
  ...
  it('can support the retrieval of transactions for an address (via 3PBP)', function (done) {
    if (process.env.BITGO_UTXO_LIB_TEST_SKIP_3PBP === '1') {
      console.warn(
        `BLOCK-253 - skip test because we can get rate limited by Cloudflare`,
      )
      this.skip()
      return done()
    }
  ...
  it('can generate a Litecoin address', function () {
    ...
    assert.strictEqual(
      wif,
      'T7A4PUSgTDHecBxW1ZiYFrDNRih2o7M8Gf9xpoCgudPF9gDiNvuS',
    )
  })
})
