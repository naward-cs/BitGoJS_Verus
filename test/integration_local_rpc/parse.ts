  ...
  describe(fixtureName, function () {
    ...
    before(async function () {
      try {
        ...
        parsedTx = utxolib.Transaction.fromBuffer(
    ...
    it('compare against RPC data', function () {
      const tx = parseTransactionRoundTrip(
        Buffer.from(fixture.transaction.hex, 'hex'),
        network,
      )
    ...
    function getRebuiltTransaction(signKeys?: bip32.BIP32Interface[]) {
      ...
      return createSpendTransactionFromPrevOutputs(
        ...
        scriptType,
        parsedTx.ins.map(i => [
          getTxidFromHash(i.hash),
          i.index,
          getPrevOutputValue(i),
        ]),
        recipientScript,
        0,
        network,
    ...
    it(`verifySignatures with one or two signatures`, function () {
      fixtureKeys.forEach(key1 => {
        ...
        rebuiltTx.ins.forEach((input, i) => {
          assert.strict(
            verifySignature(rebuiltTx, i, getPrevOutputValue(input)),
          )
        })
    ...
    it('createSpendTransaction match', function () {
      ...
      assert.strictEqual(
        rebuiltTx.toBuffer().toString('hex'),
        fixture.transaction.hex,
      )
    })
  })
}
