...
export function getKeyTriple(seed: string, network?: Network): KeyTriple {
  return [
    getKey(seed + '.0', network),
    getKey(seed + '.1', network),
    getKey(seed + '.2', network),
  ]
}
...
export function isSupportedDepositType(
  network: Network,
  scriptType: ScriptType,
): boolean {
  return (
    (!requiresSegwit(scriptType) || supportsSegwit(network)) &&
    (!isVerus(network) || scriptType !== 'p2sh')
  )
}

export function isSupportedSpendType(
  network: Network,
  scriptType: ScriptType,
): boolean {
 ...
 */
export function createScriptPubKey(
  keys: KeyTriple,
  scriptType: ScriptType,
  network: Network,
): Buffer {
...
export function createSpendTransactionFromPrevOutputs(
  ...
  if (isZcashCompatible(network)) {
    txBuilder.setExpiryHeight(height === 0 ? 0 : height + 100)
    txBuilder.setLockTime(height === 0 ? 0 : height - 10)
  }
