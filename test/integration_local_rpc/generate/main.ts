...
function toRegtestAddress(
  network: {bech32?: string},
...
async function createTransactionsForScriptType(
  ...
  await writeTransactionFixtureWithInputs(
    rpc,
    network,
    `deposit_${scriptType}.json`,
    depositTxid,
  )
  if (
    !isScriptType2Of3(scriptType) ||
    !isSupportedSpendType(network, scriptType)
  ) {
...
async function main(args: string[]) {
  ...
  for (const networkName of Object.keys(utxolib.networks)) {
    ...
    if (
      ...
      !allowedNetworks.some(n => n === getMainnet(network))
    ) {
