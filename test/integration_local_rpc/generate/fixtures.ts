...
export async function writeFixture(
  ...
  await fs.writeFile(
    path.join(getFixtureDir(network), filename),
...
export async function readFixture<T>(
  ...
  return JSON.parse(
    await fs.readFile(path.join(getFixtureDir(network), filename), 'utf8'),
  )
}
...
export async function writeTransactionFixtureWithInputs(
  ...
  const inputTransactionIds = transaction.vin.reduce(
    (all: string[], input) =>
      all.includes(input.txid) ? all : [...all, input.txid],
    [],
  )
  const inputs = await Promise.all(
    inputTransactionIds.map(inputTxid =>
