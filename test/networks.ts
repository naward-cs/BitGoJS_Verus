...
describe('networks', function () {
  ...
  const verusSharedMessagePrefix = network =>
    coins.isVerus(network) || coins.isPBaaS(network)

  const verusSharedScriptPrefix = network =>
    coins.isVerus(network) || coins.isPBaaS(network) || coins.isKomodo(network)

  const verusSharedPubkeyPrefix = network =>
    coins.isPBaaS(network) || coins.isKomodo(network)

  const dogeSharedPubkeyPrefix = network =>
    coins.isDigibyte(network) || coins.isDoge(network)

  const verusSharedWIFPrefix = network =>
    coins.isPBaaS(network) || coins.isVerus(network) || coins.isKomodo(network)
  ...
  for (const name in networks) {
    ...
    describe(`networks.${name}`, function () {
      ...
      it('has expected properties', function () {
        ...
        assert.strictEqual(
          typeof network.bech32,
          bech32Coins(network) ? 'string' : 'undefined',
        )
        ...
        if (bip32PBaaSCompatible(network)) {
          ...
          assert.strictEqual(
            ...
            networks.bitcoin.bip32.private,
          )
          ...
          assert.strictEqual(
            network.bip32.private,
            networks.testnet.bip32.private,
          )
        }
      })
      ...
      for (const otherName in networks) {
        ...
        it(`has distinct properties with ${otherName}`, function () {
          ...
          assert.strictEqual(
            ...
            sameGroup(
              bitcoinMainnetSharedPubkeyPrefix,
              network,
              otherNetwork,
            ) ||
              sameGroup(
                bitcoinTestnetSharedPubkeyPrefix,
                network,
                otherNetwork,
              ) ||
          ...
          assert.strictEqual(
            ...
            sameGroup(
              bitcoinMainnetSharedScriptPrefix,
              network,
              otherNetwork,
            ) ||
              sameGroup(
                bitcoinTestnetSharedScriptPrefix,
                network,
                otherNetwork,
              ) ||
