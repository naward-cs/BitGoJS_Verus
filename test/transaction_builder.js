...
var {
  createTransactionForNetwork,
  createTransactionBuilderForNetwork,
  createTransactionBuilderFromTransaction,
} = require('../src/bitgo/transaction')

var fixtures = require('./fixtures').combine('transaction_builder', [
  'dash',
  'zcash',
])
...
function construct(f, dontSign) {
  ...
  if (f.versionGroupId !== undefined)
    txb.setVersionGroupId(parseInt(f.versionGroupId, 16))
  if (f.consensusBranchId !== undefined)
    txb.setConsensusBranchId(parseInt(f.consensusBranchId, 16))
...
describe('TransactionBuilder', function () {
  ...
  networksToTest.forEach(function (network) {
    describe('addInput for ' + network, function () {
      ...
      var txb
      var testKeyPair = new ECPair(BigInteger.ONE, undefined, {
        network: testNetwork,
      })
      beforeEach(function () {
        txb = createTransactionBuilderForNetwork(testNetwork)
        if (coins.isZcashCompatible(testNetwork)) {
          txb.setVersion(3)
        }
      })
  ...
  describe('sign', function () {
    it('supports the alternative abstract interface { publicKey, sign }', function () {
      ...
      assert.equal(
        txb.build().toHex(),
        '',
      )
    })
  ...
  describe('build', function () {
    ...
    it('for incomplete P2WSH with 0 signatures', function () {
      var inpTx = Transaction.fromBuffer(
        Buffer.from(
          ...
          input.signs.forEach(function (sign) {
            ...
            if (tx) {
              ...
              if (sign.filterOP_0) {
                ...
                var replacement = btemplates.scriptHash.input.encode(
                  btemplates.multisig.input.encode(signatures),
                  redeemScript,
                )
                assert.strictEqual(
                  bscript.toASM(replacement),
  ...
  describe('various edge case', function () {
    ...
    it('should classify witness inputs with witness = true during multisigning', function () {
      ...
      var txHex = tx.toHex()
      var newTxb = TransactionBuilder.fromTransaction(
    ...
    it('should handle badly pre-filled OP_0s', function () {
      ...
      var redeemScript = bscript.fromASM(
